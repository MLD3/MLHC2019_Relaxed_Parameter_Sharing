...
class pytorchLSTM_jiaxuan(nn.Module):

    def __init__(self, args):
        from MTL_RNN.lib.model import RNN_LSTM
        #super(self.__class__, self).__init__()
        nn.Module.__init__(self)
        self.args  = args
        self.model = RNN_LSTM(args['d'], args['hidden_size'],
                              args['output_size'],
...
class moeLSTM(nn.Module): # individualized
...
class LSTM_jiaxuan(nn.Module):
    ...
    def forward(self, input, hidden, i_, f_, c_, o_):
        ...
        return yhat.view(seq_len, bs, self.args['output_size']), hidden
...
class ChangeGate_moo(mooLSTM):
    ...
    def __init__(self, args):
        ...
        # both o and (h, c)
        self.shared_model = LSTM_jiaxuan(args, activation=nn.ReLU())
...
class AttentionBlockT(nn.Module):
    ...
    def forward(self, minibatch):
        minibatch = minibatch.permute(0,2,1)
            ...
            #joh: nn.Linear Input=(N,*,in_features), Output=(N,*,out_features)
            #joh: for each timestep T, returns a linear combination of the channel values.
        t_array = (torch.arange(1,minibatch.shape[1]+1)/minibatch.shape[1]).repeat(minibatch.shape[0],1).unsqueeze(2).float()
...
class SNAIL(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.out_layer = nn.Conv1d(args['d']+args['kvdims']*6+2*args['num_filters']*layer_count, args['output_size'], 1) #in_channels, out_channels, kernel_size
...
class AttentionBlockTE(nn.Module):
    """An attention mechanism similar to Vaswani et al (2017)
    ...
    JOH: This version adds a dimension for t in the inpust of key_layer and query_layer
    """
    def __init__(self, input_dims, k_size, v_size, args):
        super(AttentionBlockTE, self).__init__()
        self.key_layer = nn.Linear(input_dims+args['te_size'], k_size)
        self.query_layer = nn.Linear(input_dims+args['te_size'], k_size)
        self.value_layer = nn.Linear(input_dims, v_size)
            #joh: nn.Linear(in_features,out_features)
        self.sqrt_k = math.sqrt(k_size)
    ...
    def forward(self, minibatch):
        minibatch = minibatch.permute(0,2,1)
            ...
            #joh: nn.Linear Input=(N,*,in_features), Output=(N,*,out_features)
            #joh: for each timestep T, returns a linear combination of the channel values.
        t_array = self.te.unsqueeze(0).repeat(minibatch.shape[0],1,1)
        minibatchT = torch.cat([minibatch,Variable(t_array.cuda())], dim=2) 
            #joh: version of minibatch with extra dimension for t
        keys = self.key_layer(minibatchT) #joh: (N,T,k_size)
        queries = self.query_layer(minibatchT) #joh: (N,T,k_size)
...
class SNAILTE(nn.Module):

    def __init__(self, args):
        super(SNAILTE, self).__init__()
        layer_count = math.ceil(math.log(args['T'])/math.log(2))
        
        self.mod0 = AttentionBlockTE(args['d'], args['kvdims'], args['kvdims'], args) #input size , kdims, vdims
        self.mod1 = TCBlock(args['d']+args['kvdims'], args['T'], args['num_filters']) # in_channels, seq_len, filters
        
        self.mod2 = AttentionBlockTE(args['d']+args['kvdims']+args['num_filters']*layer_count, 2*args['kvdims'], 2*args['kvdims'], args)
        self.mod3 = TCBlock(args['d']+args['kvdims']*3+args['num_filters']*layer_count, args['T'], args['num_filters'])
        self.mod4 = AttentionBlockTE(args['d']+args['kvdims']*3+2*args['num_filters']*layer_count, 3*args['kvdims'], 3*args['kvdims'], args)
...
class DenseBlockID(nn.Module):
    ...
    joh: For idTCN
        Need to pass in T. Learns separate filters for each timestep
    """
    def __init__(self, in_channels, filters, T, dilation=1):
        super(DenseBlockID, self).__init__()
        self.T = T
    ...
    def cutup(self,data):
        #Input (B, T*F, T), Output (B, F, T)
        newdata = data[:,:self.filters,0].unsqueeze(2)
        for t in range(1,self.T):  
            start = t*self.filters
            end = start + self.filters
            newdata = torch.cat([newdata,data[:,start:end,t].unsqueeze(2)], dim=2)
        return newdata
        
    def forward(self, minibatch):
        tanh = torch.tanh(self.cutup(self.causal_conv1(minibatch)))
        sig = torch.sigmoid(self.cutup(self.causal_conv2(minibatch)))
        out = torch.cat([minibatch, tanh*sig], dim=1)
        return out


class TCBlockID(nn.Module):
    """A stack of DenseBlocks which dilates to desired sequence length
    ...
    """
    def __init__(self, in_channels, seq_len, filters):
        super(TCBlockID, self).__init__()
        layer_count = math.ceil(math.log(seq_len)/math.log(2))
        blocks = []
        channel_count = in_channels
        for layer in range(layer_count):
            block = DenseBlockID(channel_count, filters, seq_len, dilation=2**layer)
            blocks.append(block)
            channel_count += filters
        self.blocks = nn.Sequential(*blocks)
...
class TCN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.out_layer = nn.Conv1d(args['d']+args['num_filters']*layer_count, args['output_size'], 1) #in_channels, out_channels, kernel_size
...
class idTCN(nn.Module):

    def __init__(self, args):
        super(idTCN, self).__init__()
        layer_count = math.ceil(math.log(args['T'])/math.log(2))

        self.mod0 = TCBlockID(args['d'], args['T'], 10) # in_channels, seq_len, filters        
        self.out_layer = nn.Conv1d(args['d']+10*layer_count, args['output_size'], 1) #in_channels, out_channels, kernel_size
