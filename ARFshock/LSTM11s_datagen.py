...
'''Modified 12 hour prediction task'''
class real_data12(data.Dataset):
    ...
    def __init__(self, mode, args, sample_ind=None):
        ...
        if args['T']!=12: raise ValueError('T is not set to 12!')
        ...
        # Bring in ARF/Shock Labels
        if 'ARF' in args['mode']:
            if mode=='train': new_labels = pd.read_hdf(savepath+'train_ARF_hr12.h5','df')
            if mode=='val': new_labels = pd.read_hdf(savepath+'val_ARF_hr12.h5','df')
            if mode=='test': new_labels = pd.read_hdf(savepath+'test_ARF_hr12.h5','df')
        if 'Shock' in args['mode']:
            if mode=='train': new_labels = pd.read_hdf(savepath+'train_Shock_hr12.h5','df')
            if mode=='val': new_labels = pd.read_hdf(savepath+'val_Shock_hr12.h5','df')
            if mode=='test': new_labels = pd.read_hdf(savepath+'test_Shock_hr12.h5','df')
        ...
        if (args['N'] > len(data_raw['labels'])) & (mode=='train'): 
            raise ValueError('Chosen N is larger than training set size')
        if (mode=='train') & (args['N']!=0):
            print('--Subsampling Occurring ~ did you expect this.?')
            if sample_ind is None:
                self.sample_ind = np.random.choice(len(data_raw['labels']), size=args['N'], replace=False)
            elif len(sample_ind)!=args['N']:
                raise ValueError('Length of sample_ind != N')
                ...
                self.sample_ind=sample_ind
            self.data = torch.from_numpy(data_raw['data'][self.sample_ind,:12,:]).type(torch.FloatTensor).cuda()
            self.labels = torch.from_numpy(data_raw['labels'][self.sample_ind]).type(torch.LongTensor).cuda()

        else:
            self.data = torch.from_numpy(data_raw['data'][:,:12,:]).type(torch.FloatTensor).cuda()
            self.labels = torch.from_numpy(data_raw['labels']).type(torch.LongTensor).cuda()
            self.sample_ind = np.arange(len(data_raw['labels']))
