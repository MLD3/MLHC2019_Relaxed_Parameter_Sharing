...
class DenseBlockID(nn.Module):
    ...
    def forward(self, minibatch):
        tanh = torch.tanh(self.causal_conv1_bn(self.cutup(self.causal_conv1(minibatch))))
        sig = torch.sigmoid(self.causal_conv2_bn(self.cutup(self.causal_conv2(minibatch))))
        out = torch.cat([minibatch, tanh*sig], dim=1)
        return out
...
class idTCN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mod0 = TCBlockID(args['d'], args['T'], args['num_filters']) # in_channels, seq_len, filters        
        self.out_layer = nn.Conv1d(args['d']+args['num_filters']*layer_count, args['num_classes'], 1) #in_channels, out_channels, kernel_size
